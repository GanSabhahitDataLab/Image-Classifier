...
def arg_parser():
    ...
    parser.add_argument('--arch', 
                        type=str, 
                        help='Select architecture from torchvision.models')
    
    # Add checkpoint directory to parser
    parser.add_argument('--save_dir', 
                        type=str, 
                        help='Enter save directory for checkpoints.')
    
    # Add hyperparameter tuning to parser
    parser.add_argument('--learning_rate', 
                        type=float, 
                        help='Enter gradient descent learning rate as float')
...
# primaryloader_model(architecture="vgg19") downloads model (primary) from torchvision
...
def initial_checkpoint(Model, Save_Dir, Train_data):
    ...
    if type(Save_Dir) == type(None):
        print("Model checkpoint directory not specified, so model will not be saved.")
    else:
        if isdir(Save_Dir):
